    ...
    <>
      <section className="px-5 py-20 lg:px-8">
        <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-2">
          ...
          <Reveal delay={120}>
            <SectionHeading align="left" eyebrow={content.heritageEyebrow} title={content.heritageTitle} />
            <p className="mt-5 leading-relaxed text-muted-foreground">{content.heritageText}</p>
            <Button asChild className="mt-7 rounded-full bg-primary px-6 text-primary-foreground hover:bg-primary/90">
              <Link href="/about">
                {t('home.seeMore')} <ArrowRight className="size-4" />
              </Link>
            </Button>
          </Reveal>
        </div>
      ...
      <section className="bg-secondary/60 px-5 py-20 lg:px-8">
        <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-2">
          <Reveal>
            <SectionHeading align="left" eyebrow={content.storyEyebrow} title={content.storyTitle} />
            {content.storyParagraphs.map((paragraph, index) => (
              <p
                key={paragraph.slice(0, 28)}
                className={index === 0 ? 'mt-5 leading-relaxed text-muted-foreground' : 'mt-4 leading-relaxed text-muted-foreground'}
              >
                {paragraph}
              </p>
            ))}
            <Button asChild variant="outline" className="mt-7 rounded-full border-foreground/20 px-6 hover:bg-foreground/5">
              <Link href="/about">
                {t('home.readMore')} <ArrowRight className="size-4" />
              </Link>
            </Button>
          </Reveal>
      ...
      <section className="px-5 py-20 lg:px-8">
        <Reveal>
          <SectionHeading eyebrow={t('home.whyBeinzirt')} title={t('home.craftedFor')} />
        </Reveal>
            ...
            <Reveal key={product.id} delay={(index % 4) * 80}>
              ...
              >
                <div className="relative aspect-square overflow-hidden">
                  ...
                  />
                  <span className="absolute left-3 top-3 rounded-full border border-[#f4d58a]/45 bg-[#2f281f]/95 px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-wider text-[#f4d58a] shadow-lg backdrop-blur">
                    {t('product.bestSeller')}
                  </span>
                  {product.availability === false ? (
                    <span className="absolute right-3 top-3 rounded-full border border-background/25 bg-foreground/85 px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-wider text-background shadow-lg backdrop-blur">
